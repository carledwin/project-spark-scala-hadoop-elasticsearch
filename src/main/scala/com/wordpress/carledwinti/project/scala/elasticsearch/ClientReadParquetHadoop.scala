...
object ClientReadParquetHadoop {
  def main(args: Array[String]): Unit = {
    ...
    val motoDF = parquetReader(spark, pathParquetFile)
    spark.close
  }

  def parquetReader(spark: SparkSession, pathParquetFile: String): DataFrame = {
    val motoParquetDf = spark.read.parquet(pathParquetFile + "/parquet/moto.parquet").toDF()
    motoParquetDf.show
    motoParquetDf
  }
}